...
trait FileUploadTrait
{
            ...
            foreach ($request->all() as $key => $value) {
                if ($request->hasFile($key)) {
                    if ($request->has($key . '_max_width') && $request->has($key . '_max_height')) {
                        ...
                        // Resize based on given width/height
                        $maxW = $request->{$key . '_max_width'};
                        $maxH = $request->{$key . '_max_height'};
                        ...
                        if ($width > $maxW && $height > $maxH) {
                            ...
                            $image->resize($maxW, null, function ($constraint) {
                                $constraint->aspectRatio();
                            });
                        } elseif ($height > $maxH) {
                            $image->resize(null, $maxH, function ($constraint) {
                                $constraint->aspectRatio();
                            });
                        }
                        ...
                        /**
                         * ---------------------------------------------------
                         * Upload resized main image to S3 using your helper
                         * ---------------------------------------------------
                         */
                        $tempMainPath = sys_get_temp_dir() . '/' . $filename;
                        $image->save($tempMainPath);

                        $mainUrl = CustomHelper::uploadToS3(new \Illuminate\Http\File($tempMainPath), $filename, 'uploads');

                        /**
                         * ---------------------------------------------------
                         * Upload thumbnail to S3 using your helper
                         * ---------------------------------------------------
                         */
                        $thumbName = 'thumb-' . $filename;
                        $thumbTempPath = sys_get_temp_dir() . '/' . $thumbName;

                        Image::make($file)->resize(50, 50)->save($thumbTempPath);

                        $thumbUrl = CustomHelper::uploadToS3(new \Illuminate\Http\File($thumbTempPath), $thumbName, 'uploads/thumb');

                        // Return both URLs in request
                        $finalRequest = new Request(array_merge($finalRequest->all(), [
                            $key => $mainUrl,
                            $key . '_thumb' => $thumbUrl
                        ]));
    ...
    public function saveAllFiles(Request $request, $downloadable_file_input = null, $model_type = null, $model = null)
    {
        ...
        try {
            ...
            if (!file_exists(public_path('storage/uploads'))) {
                mkdir(public_path('storage/uploads'), 0777, true);
                mkdir(public_path('storage/upload/thumb'), 0777, true);
            }
            $finalRequest = $request;
    ...
    public function saveAllFilesByLesson($files, $downloadable_file_input = null, $model_type = null, $model = null, $pointer, $media = null)
    {
        //dd("lesson-2");
        try {
