...
use App\Models\{Reason, Events, Announcement};
use CustomHelper;
...
class AnnouncementController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        Announcement::populateSlugs();
        $reasons = Announcement::orderBy('created_at', 'desc')->where('lang', 'en')->get();
        ...
        return DataTables::of($reasons)
            ...
            ->addColumn('actions', function ($q) use ($has_view, $has_edit, $has_delete, $request) {
                ...
                if ($request->show_deleted == 1) {
                    return view('backend.datatable.action-trashed')->with(['route_label' => 'admin.announcement', 'label' => 'news', 'value' => $q->id]);
                }

                if ($has_edit) {
                    $edit = view('backend.datatable.action-edit')
                        ->with(['route' => route('admin.announcement.edit', ['page' => $q->id]) . "?slug=$q->slug"])
                        ->render();
                    $view .= $edit;
                }
                ...
                if ($has_delete) {
                    $delete = view('backend.datatable.action-delete')
                        ->with(['route' => route('admin.announcement.destroy', ['page' => $q->id])])
                        ->render();
                    $view .= $delete;
                }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.announcement.index')->withFlashSuccess(trans('alerts.backend.general.created'));
    }
    ...
    public function edit($id)
    {
        ...
        $slug = request()->slug;
        $reason = Announcement::where('slug', $slug)->where('lang', $lang)->first();

        if (!$reason) {
            $existing = Announcement::where('slug', $slug)->where('lang', 'en')->first();
            $reason = new Announcement;
            $reason->lang = $lang;
        ...
        return view('backend.announcement.edit', compact('reason'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $reason = Announcement::findOrFail($id);
        $reason->title = $request->title;
        $reason->event_date = date('Y-m-d H:i:s', strtotime($request->event_date));
        $reason->content = $request->get('content');
        if ($request->news_image != "") {
        ...
        return redirect()->route('admin.announcement.index')->withFlashSuccess(trans('alerts.backend.general.updated'));
    }
    ...
    public function show($id)
    {
        ...
        return view('backend.announcement.show', compact('reason'));
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('admin.announcement.index')->withFlashSuccess(trans('alerts.backend.general.deleted'));
    }
    ...
    {
        $reason = Announcement::findOrFail(request('id'));
